...
describe('screen4 runway component', () => {
  ...
  it('keeps unfinished tasks static with running tasks first', () => {
    ...
    expect(template).toContain('class="ticker-chip"')
    expect(template).toContain('v-for="(step, index) in displayedSteps"')
    ...
    expect(template).toContain('class="chip-head"')
    expect(template).toContain('{{ formatTickerChipIndex(index) }}')
    expect(template).toContain('class="chip-operator"')
    expect(template).toContain('{{ tickerOperatorText(step) }}')
    expect(template).toContain("'is-unassigned'")
    expect(template).not.toContain('class="chip-operator-avatar"')
    expect(template).not.toContain('tickerOperatorInitial(step)')
    expect(source).toContain("return step.operator?.trim() || '未指派'")
    ...
    expect(template).toContain('ref="tickerViewportRef"')
    expect(source).toContain('getScreen4TickerVisibleLimit(')
    expect(source).toContain('new ResizeObserver(updateTickerVisibleLimit)')
    expect(source).toContain('visibleSteps.value.slice(0, tickerVisibleLimit.value)')
    expect(template).toContain('class="ticker-more"')
    expect(template).toContain('{{ tickerHiddenCount }}')
    expect(styles).toContain('.ticker-more')
    ...
    // 卡片宽度自适应任务名长度（≤25 字完整、超出截断省略），行内余白由传送带车道延伸。
    expect(styles).toMatch(/\.ticker-chip\s*\{[^}]*flex: 0 1 auto;[^}]*max-width: 560px;/s)
    expect(styles).toMatch(/\.ticker-track\s*\{[^}]*&::after\s*\{[^}]*animation: standby-drift/s)
    expect(styles).toMatch(/\.chip-index\s*\{[^}]*border-left: 1px solid/s)
