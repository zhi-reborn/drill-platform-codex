...
describe('screen4 runway component', () => {
  ...
  it('keeps unfinished tasks static with running tasks first', () => {
    ...
    expect(template).toContain('当前环节待完成任务')
    expect(template).toContain('<strong>{{ visibleSteps.length }}</strong>')
    expect(template).toContain('<em>项</em>')
    expect(template).not.toContain('{{ tickerDoneCount }}/{{ runningSteps.length }}')
    ...
    expect(source).not.toContain('const tickerDoneCount = computed')
