...
const screen = readFileSync(resolve(root, 'src/views/ScreenView.vue'), 'utf8')
assert.match(screen, /<button[\s\S]*?class="btn-icon btn-fullscreen-mark"[\s\S]*?@click="toggleFullscreen"/, 'ScreenView 全屏按钮必须保留并可切换全屏')
assert.match(screen, /screen-brand\.json/, 'ScreenView 品牌信息必须从运行时 screen-brand.json 读取')
assert.match(screen, /screenBrand\.companyName/, 'ScreenView 公司名称必须绑定运行时品牌配置')
assert.match(screen, /screenBrand\.fullscreenIconImage/, 'ScreenView 全屏按钮必须支持运行时图片图标配置')
assert.match(screen, /screenBrand\.fullscreenIconText/, 'ScreenView 全屏按钮必须支持运行时文字图标配置')
assert.match(screen, /\.header-brand\s*\{[\s\S]*?font-size:\s*16px[\s\S]*?color:\s*#f0f7ff/, 'ScreenView 右上角公司名称必须使用更大字号和 KPI 标题同级白色')
