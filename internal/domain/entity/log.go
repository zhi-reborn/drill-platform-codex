...
type DrillInstanceLog struct {
	...
	StepInstanceID  *uint64   `gorm:"type:bigint unsigned;column:task_instance_id;index:idx_step_instance;uniqueIndex:uk_log_command_action_task,priority:3" json:"step_instance_id"`
	CommandID       *uint64   `gorm:"type:bigint unsigned;column:command_id;uniqueIndex:uk_log_command_action_task,priority:1" json:"command_id,omitempty"`
	Action          string    `gorm:"type:varchar(32);not null;column:action;uniqueIndex:uk_log_command_action_task,priority:2" json:"action"`
