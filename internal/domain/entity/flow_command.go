...
type FlowCommand struct {
	ID              uint64            `gorm:"primaryKey;autoIncrement;column:id;index:idx_flow_command_pending,priority:3;index:idx_flow_command_drill_status,priority:3" json:"id"`
	CommandType     string            `gorm:"type:varchar(64);not null;column:command_type" json:"command_type"`
	DrillInstanceID uint64            `gorm:"type:bigint unsigned;not null;column:drill_instance_id;index:idx_flow_command_drill_status,priority:1" json:"drill_instance_id"`
	...
	Status          FlowCommandStatus `gorm:"type:varchar(20);not null;index:idx_flow_command_pending,priority:1;index:idx_flow_command_lease,priority:1;index:idx_flow_command_drill_status,priority:2;column:status" json:"status"`
