...
func SetupRouter(services *service.Services, wsManager *websocket.Manager, jwtSecret string) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		v1.Use(jwtAuth)
		{
			...
			v1.POST("/templates", middleware.RequireDirectorOrAbove(), templateHandler.Create)
			...
			v1.GET("/drills/:id/steps", drillHandler.GetSteps)
			v1.GET("/drills/:id/logs", drillHandler.GetLogs)
			...
			v1.POST("/drills/:id/steps/start", middleware.RequireDirectorOrAbove(), drillHandler.StartStep)
