...
// 云原生系统故障应急演练模板种子脚本（环节名统一 12 字）
//
// 用法：
//   1. 确保后端服务已启动（默认监听 http://localhost:8080）
//   2. 运行： go run cmd/seed-template-twelve/main.go
//   3. 指向其他后端： BASE_URL=http://host:port/api/v1 go run cmd/seed-template-twelve/main.go
//
// 模板结构（符合规格）：
//   - 4 个阶段，每阶段 2-16 个环节（实际 3-5 个）
//   - 每个环节名统一 12 个字（脚本启动时内置校验，不满足则报错退出）
...
func main() {
	...
	// generateSteps 内部会校验所有环节名均为 12 字
...
func createTemplate(token string) (int, error) {
	body, _ := json.Marshal(map[string]string{
		"name":        templateName,
		"category":    "灾备",
		"description": "面向云原生系统的4阶段故障应急演练模板，环节名统一12字，覆盖故障感知、应急响应、处置恢复、复盘改进，阶段与环节串行，任务/子任务串并行混合",
	})
	...
	phases := []phaseDef{
		// 阶段1：故障感知与预警（4 环节，每环节名 12 字）
		{
			...
			stages: []stageDef{
				...
				{name: "业务影响的评估与等级判定", stepType: "serial", tasks: []taskDef{
				...
				{name: "内外部信息通报与分发处理", stepType: "serial", tasks: []taskDef{
		...
		// 阶段2：应急响应启动（3 环节，每环节名 12 字）
		{
			name: "应急响应启动", stepType: "serial",
			stages: []stageDef{
				{name: "应急小组的组建与职责分配", stepType: "serial", tasks: []taskDef{
				...
				{name: "故障根因的研判与方案制定", stepType: "serial", tasks: []taskDef{
				...
				{name: "应急资源的调度与人员到位", stepType: "serial", tasks: []taskDef{
		...
		// 阶段3：故障处置恢复（5 环节，每环节名 12 字）
		{
			name: "故障处置恢复", stepType: "serial",
			stages: []stageDef{
				{name: "故障节点的隔离与扩散阻断", stepType: "serial", tasks: []taskDef{
				...
				{name: "备用容量的确认与流量切换", stepType: "serial", tasks: []taskDef{
				...
				{name: "核心服务的恢复与健康验证", stepType: "serial", tasks: []taskDef{
				...
				{name: "数据一致性校验与修复处理", stepType: "serial", tasks: []taskDef{
				...
				{name: "全量流量的回切与监控确认", stepType: "serial", tasks: []taskDef{
		...
		// 阶段4：复盘总结改进（3 环节，每环节名 12 字）
		{
			name: "复盘总结改进", stepType: "serial",
			stages: []stageDef{
				{name: "故障时间线复盘与梳理分析", stepType: "serial", tasks: []taskDef{
				...
				{name: "根因的深度定位与改进梳理", stepType: "serial", tasks: []taskDef{
				...
				{name: "改进措施的落地与预案更新", stepType: "serial", tasks: []taskDef{
